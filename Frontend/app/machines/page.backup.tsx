      ...
      <div className="container mx-auto p-6">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-2">
          {machines.map((machine) => {
            const fp = machine.failure_prediction
            return (
            <Card key={machine.id} className="hover:shadow-lg transition-shadow">
              ...
              <CardContent className="space-y-4">
                ...
                {/* Failure Prediction */}
                {fp ? (
                  <div className={`p-4 rounded-lg ${getRiskLevelColor(fp.risk_level)}`}>
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-semibold">Failure Prediction</h4>
                      <Badge variant="outline" className={getRiskLevelColor(fp.risk_level)}>
                        {getRiskLevelText(fp.risk_level)} Risk
                      </Badge>
                    </div>
                    <div className="space-y-2">
                      <div className="flex items-center justify-between text-sm">
                        <span>Risk Level:</span>
                        <span className="font-semibold">{fp.risk_level}%</span>
                      </div>
                      <Progress value={fp.risk_level} className="h-2" />
                      <div className="text-sm">
                          <strong>Predicted Failure:</strong> {fp.predicted_failure_type.replace(/_/g, ' ')}
                      </div>
                      <div className="text-xs text-gray-600">
                        {fp.failure_description}
                      </div>
                      <div className="text-sm pt-2">
                        <strong>Maintenance Priority:</strong> {fp.maintenance_priority}
                      </div>
                      <div className="text-xs text-gray-700">
                        <strong>Recommended Action:</strong> {fp.recommended_action}
                      </div>
                      <div className="text-sm">
                        <strong>Est. Time to Failure:</strong> {fp.estimated_time_to_failure}
                      </div>
                      <div className="text-xs">
                        <strong>Confidence:</strong> {fp.confidence}%
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="p-4 rounded-lg bg-gray-50 text-gray-600">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-semibold">Failure Prediction</h4>
                      <Badge variant="outline" className="text-gray-600">No Data</Badge>
                    </div>
                    <div className="text-sm">No prediction available</div>
                  </div>
                )}
